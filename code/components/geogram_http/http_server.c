...
static const char *LANDING_PAGE_HTML_SUFFIX =
    ...
    "(function(){"
    "const preferred='geogram.local';"
    "if(location.hostname!==preferred&&location.hostname!=='192.168.4.1'){"
    "location.replace('http://'+preferred+'/');"
    "}"
    "})();"
    ...
    "$('status').textContent='Nostr extension denied';"
    "await reportClient({error:msg});"
    "}"
    "}"
