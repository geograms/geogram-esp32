...
static const char *LANDING_PAGE_HTML_PREFIX =
    ...
    ".header{border-bottom:1px solid var(--border);padding:12px;display:flex;align-items:center;gap:12px;position:relative}"
    ...
    "<button id=\"connectBtn\">Connect Nostr extension</button>"
...
static const char *LANDING_PAGE_HTML_SUFFIX =
    ...
    "try{await connectExtension();if(clientKeys&&clientKeys.mode==='extension'){return;}}catch(e){}"
    "}"
    ...
    "if(!window.nostr||!window.nostr.signEvent){$('status').textContent='No Nostr extension';return;}"
    "try{"
    "const clientTs=Math.floor(Date.now()/1000);"
    "const event={kind:1,content:'geogram connect',created_at:clientTs,tags:[[\"client\",\"geogram\"]]};"
    "const signed=await window.nostr.signEvent(event);"
    "if(signed&&signed.pubkey){"
    "clientKeys={npub:'',callsign:'',mode:'extension',pubkey:signed.pubkey};"
    "ensureNpubAndCallsign();"
    "updateStatus();"
    "await reportClient(clientKeys);"
    "}"
    "}catch(e){$('status').textContent='Extension declined';}"
    "}"
    ...
    "if(window.nostr&&window.nostr.signEvent){connectExtension();}"
    "}"
    "function initMenu(){"
    "const btn=$('menuBtn');"
    ...
    "});"
    "$('connectBtn').onclick=()=>{panel.classList.remove('open');connectExtension();};"
